<template>
  <div class="wrapper">
    ...
    <div class="box pid">
      ...
      <Checkbox ref="pidmode" v-bind:name="(pidMode) ? 'Sample Acquisition' : 'Robotic Arm'" v-on:toggle="updatePIDMode($event)"/>
      <br>
  ...
  export default {
    ...
    data () {
      return {
        ...
        Ki: 0.0,
        Kd: 0.0,
        pidMode: false, // true = sa, false = ra
        controlMode: 0,
        demand: 0,
        chartData: [
          { data: [],
    ...
    methods: {
      ...
      updatePIDMode: function (checked) {
        if (checked) {
          this.pidMode = true
        } else {
          this.pidMode = false
